...
export default function ReviewPage() {
  ...
  if (response.status !== "concluido" || !value) return <Container maxWidth={false} sx={{ height: "100dvh", py: 1.5, display: "flex", flexDirection: "column", gap: 1.5 }}>{toolbar}<Alert severity={response.status === "ILEGIVEL_PARA_REVISAO_MANUAL" ? "warning" : "error"}>{response.status === "erro" || response.status === "ILEGIVEL_PARA_REVISAO_MANUAL" ? response.erro : "A transcrição ainda está sendo processada."}</Alert></Container>;
