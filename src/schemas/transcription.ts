...
export const holeriteFieldSchema = z.object({ code: z.string(), label: observed, reference: z.string(), value: observed });
export const holeriteBaseSchema = z.object({ label: observed, value: observed });
export const holeriteValueSchema = z.object({ pages: z.array(z.object({ page: z.number().int().positive(), year: observed, month: observed, fields: z.array(holeriteFieldSchema), bases: z.array(holeriteBaseSchema) })) });
