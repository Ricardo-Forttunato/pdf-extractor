...
export async function validatePdf(bytes: Uint8Array): Promise<void> {
  if (!bytes.length || bytes.byteLength > env.maxUploadBytes)
    throw new PdfValidationError("O arquivo deve ser um PDF de até 10 MiB.");
  if (new TextDecoder().decode(bytes.slice(0, 5)) !== "%PDF-")
    throw new PdfValidationError("Envie um arquivo PDF válido.");
  ...
  try {
    ...
    if (!pdf.numPages) throw new Error("sem páginas");
  } catch {
    throw new PdfValidationError(
      "O PDF está corrompido, protegido ou não possui páginas.",
    );
  }
}
