...
test("exporta cartão com página e pares de colunas", () => {
  const result = cartaoExportModel({
    pages: [
      {
        page: 1,
        days: [
...
test("leva verbas, bases, referência e divergência do holerite", () => {
  const result = holeriteExportModel({
    pages: [
      {
        page: 1,
        month: "01",
        ...
        bases: [{ label: "Base INSS", value: "1,00" }],
      },
    ],
  });
  expect(result.headers).toEqual([
  ...
  expect(result.rows[0]).toMatchObject({
    "Referência documento": "01/2020",
    "Divergência cálculo": "Sim",
    Tipo: "PROVENTO",
  });
